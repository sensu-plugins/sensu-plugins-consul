...
## [Unreleased]
### Added
- allow usage of newer `rest-client` version to avoid RPM conflict with sensu-plugins-http
- `check-consul-servers.rb` and `check-consul-servers.rb` now accept `--token` argument (@pascalbourdier)
